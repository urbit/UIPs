...
Currently Vere and Ares both handle jet registrations provided by `%fast` hints. By convention and implementation, a `%fast` hint wraps the production of a core, and registers that core's *battery* (axis 2) along with a heirarchy of already-registered batteries which match a parent core (as an axis in the newly-registered core). The label provided in a `%fast` hint is appended onto the labels of the parent cores, producing a `$path` for each core registered. This allows us to recognize cores as having a particular label. This ability is fundamental to practical jetting systems.

To correctly implement core labeling with `%fast` hints, an interpreter must maintain *state*. The state directly assembled from `%fast` hints is conventionally referred to as the "cold" state (contrasted with the "hot" state, the map of labels to jets shipped with the interpreter binary, and the "warm" state, the ephemeral join between hot and cold state).
