...
- optional-peek-attributes:
  - undefined
- encoded-response: { meta-byte, total-bits, authentication-length, authentication, fragment-length, fragment }
  - meta-byte: { total-bits-length[2], authentication-length-length[1], fragment-length[5] }
    - total-bits-length: 2 bits
      - `(dec (met 0 (met 3 total-bits)))`
    - authentication-length-length: 1 bit
    - fragment-length: 5 bits
  - total-bits: message level bit-length
    - encoded in `(bex total-bits-length)` bytes
...
XX: revise length limits sane relative to each other
