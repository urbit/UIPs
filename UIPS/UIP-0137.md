...
4. If the merge window close date changes after the initial announcement, a new four-week notice period MUST begin from the meeting at which the revised date is announced, unless the change extends the window (i.e., pushes the close date further out).

5. The merge window MUST NOT close earlier than four weeks from the most recent announcement of its close date at a Core Architecture meeting.
...
The provision for resetting the clock when a window is moved earlier prevents the notice requirement from being circumvented by initially announcing a later date and then moving it up.
