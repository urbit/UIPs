...
Tests should use the test thread and associated library, optionally supplemented with `%quiz`. Test files should reside in `/tests` and follow the naming convention:
...
**Example:**  
This test verifies addition behavior, including identity and basic arithmetic.
...
**Example:**  
This test builds the `%time` agent, initializes it, pokes it, and checks for the expected `%wait` card.
...
**Example:**  
This test validates both JSON and noun conversions for the `loob` mark.
...
**Example:**  
This test corresponds to [Issue #6095](https://github.com/urbit/urbit/issues/6095), which involves a crash in the Hoon parser due to rune ordering.
